...
export class ArtstationScraper {
    ...
    return new Promise((resolve, reject) => {
      ...
      request({ uri: asset.url, followAllRedirects: true, encoding: null })
        .then(response => {
          ...
          FileSystem.writeFile(Settings.downloadLocation + asset.fileName + '.' + assetType, response.body, error => {
            if (error) {
              reject(error);
            } else {
  ...
  public resolveHashes(data: any, limit: number) {
    ...
    return new Promise((resolve, reject) => {
      let uri: string = 'https://www.artstation.com/users/' + Settings.userName + '/likes.json?page=';
  ...
  public checkHashChanges() {
    return new Promise(resolve => {
      ...
      let localCountPromise = checkFilePromise.then(this.getLocalCount.bind(this));
      let remoteCountPromise = this.getLikeCount(Settings.userName);
  
      Promise.all([checkFilePromise, localCountPromise, remoteCountPromise])
